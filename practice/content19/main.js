...
async function main() {
  ...
  webglLessonsUI.setupSlider("#Second-Pass-Camera-Z", {slide: updateSecondPassCamaraZ, min: -100, max: 100, step: 1});
  
  let teapotYRoatation = 0;
  let secondPassCameraZ = 0;
  //---------------------------------------------------//
  ...
  function drawScene()
  {
...

    {
      ...
      mat4.scale(modelMat, modelMat, [0.05, 0.05, 0.05]); //0.1 크기로 축소하는 model 행렬을 준비합니다. 
      ...
      mat4.translate(view, view, [0.0, 0.0, secondPassCameraZ]);
  ...
  // 2nd pass에서 사용할 카메라의 z값을 업데이트하는 함수
  function updateSecondPassCamaraZ(event, ui)
  {
    secondPassCameraZ = ui.value;
  }
