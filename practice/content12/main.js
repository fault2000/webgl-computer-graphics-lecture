...
async function main() {
  ...
  const response = await fetch('../../resources/models/teapot/teapot.obj'); 
  ...
  function drawScene()
  {
    ...
    // 매 프레임마다 sin값을 이용해 좌우로 이동하도록 합니다.
    translationOffset = Math.sin(rotationAngle) * 10.0;
    ...
    */
    ...
    // X축과 Y축의 동시 회전을 위해, 각 축의 회전 행렬을 따로 만들어 곱해줍니다.
    mat4.fromXRotation(xRotation, rotationAngle);
    mat4.fromYRotation(yRotation, rotationAngle);
    mat4.multiply(model, xRotation, yRotation);
